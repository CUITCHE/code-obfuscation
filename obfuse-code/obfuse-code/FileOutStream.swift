...
struct FileOutStream {
    ...
    mutating func begin() {
        ...
        _write(macro: "CO_CONFUSION_PROTOCOL")
    ...
    mutating func end() {
        ...
        text.insert("\(md5)\n\n", at: selfLocation)
        var oldAttribute = try! FileManager.default.attributesOfItem(atPath: filepath)
        oldAttribute[.immutable] = false
        try! FileManager.default.setAttributes(oldAttribute, ofItemAtPath: filepath)
        do {
            try text.write(toFile: filepath, atomically: true, encoding: String.Encoding.utf8.rawValue)
        } catch {
            print(error)
        }
        oldAttribute[.immutable] = true
        try! FileManager.default.setAttributes(oldAttribute, ofItemAtPath: filepath)
    }
}
